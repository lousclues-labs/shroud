    ...
    SIZES
        ...
        .map(|&size| {
            ...
            if icon_type == IconType::Connected {
                ...
                for y in 0..size {
                    for x in 0..size {
                        let pixel = draw_standalone_lock(x, y, center, s, &green, &green_border, &transparent);
...
fn draw_standalone_lock(x: i32, y: i32, center: i32, s: f32, fill: &[u8; 4], border: &[u8; 4], transparent: &[u8; 4]) -> [u8; 4] {
    ...
    let in_body_inner = rx.abs() <= (body_half_w - border_width) && ry >= (body_top + border_width) && ry <= (body_bottom - border_width);
    ...
    let in_shackle_inner = ry <= body_top && dist <= (shackle_outer_r - border_width * 0.8) && dist >= (shackle_inner_r + border_width * 0.8);
    ...
    let in_keyhole_slot = rx.abs() <= keyhole_slot_half_w && ry >= keyhole_slot_top && ry <= keyhole_slot_bottom;
