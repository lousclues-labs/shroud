...
If VPN Shroud crashed with the kill switch enabled, your traffic is blocked. That's the kill switch doing its job, just a bit too enthusiastically.

```bash
# Try this first
shroud ks off

# If VPN Shroud isn't responding
...
VPN Shroud only sees VPN connections that NetworkManager knows about.
...
VPN Shroud will back up corrupted configs and create a fresh one. But if you need to manually reset:
...
This happens when VPN Shroud's state diverges from NetworkManager. Usually after connecting/disconnecting via nm-applet or GNOME Settings.
