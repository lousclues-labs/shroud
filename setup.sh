...
#
# setup.sh - Installation script for VPNShroud VPN Manager
#
# Usage:
#   ./setup.sh [OPTIONS] [COMMAND]
#
# Commands:
#   install     Install VPNShroud (default)
#   update      Update existing installation
#   uninstall   Remove VPNShroud
...
preflight_checks() {
    ...
    if [ -z "${DISPLAY:-}" ] && [ -z "${WAYLAND_DISPLAY:-}" ]; then
        ...
        warn "VPNShroud requires a graphical environment for the system tray"
    fi
    
    # Check NetworkManager is running
    if ! systemctl is-active --quiet NetworkManager; then
        warn "NetworkManager is not running"
        warn "VPNShroud requires NetworkManager to manage VPN connections"
    else
...
stop_existing() {
    info "Stopping existing VPNShroud instance..."
...
create_default_config() {
    ...
    cat > "$config_file" << 'EOF'
# VPNShroud VPN Manager Configuration
...
# Automatically connect to the last used VPN when VPNShroud starts
...
# VPNShroud periodically checks if the VPN connection is working
...
cleanup_deprecated_systemd() {
    ...
    if [[ -f "$service_file" ]]; then
        ...
        echo "  VPNShroud now uses XDG autostart instead."
...
# VPNShroud completions for fish
...
install_sudoers() {
    ...
    echo "└─────────────────────────────────────────────────────────────────┘"
    echo ""
    echo "  VPNShroud's kill switch needs root privileges to manage firewall"
...
do_install_sudoers() {
    ...
    temp_file=$(mktemp)
    cat > "$temp_file" << EOF
# VPNShroud VPN Kill Switch - Sudoers Configuration
# Version: 3
# Generated by setup.sh on $(date)
#
# SECURITY: Rules are scoped to VPNShroud's chain operations where possible.
...
show_summary() {
    ...
    echo -e "${BOLD}${GREEN}            VPNShroud installation complete!               ${NC}"
    echo -e "${BOLD}${GREEN}════════════════════════════════════════════════════════${NC}"
    echo
...
do_uninstall() {
    echo -e "${BOLD}${RED}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}${RED}                  VPNShroud Uninstallation                     ${NC}"
    echo -e "${BOLD}${RED}═══════════════════════════════════════════════════════════${NC}"
    echo
    
    if ! confirm "Are you sure you want to uninstall VPNShroud?"; then
    ...
    echo
    echo -e "${GREEN}VPNShroud has been uninstalled.${NC}"
}
...
do_install() {
    echo -e "${BOLD}${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}${BLUE}                  VPNShroud Installation                       ${NC}"
    echo -e "${BOLD}${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo
...
do_update() {
    echo -e "${BOLD}${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}${BLUE}                    VPNShroud Update                           ${NC}"
    echo -e "${BOLD}${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo
...
do_check() {
    echo -e "${BOLD}Checking VPNShroud dependencies...${NC}"
    echo
...
do_repair() {
    echo -e "${BOLD}${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}${BLUE}                    VPNShroud Repair                           ${NC}"
    echo -e "${BOLD}${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo
...
show_status() {
    echo -e "${BLUE}╔═══════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║         VPNShroud Installation Status        ║${NC}"
    echo -e "${BLUE}╚═══════════════════════════════════════════╝${NC}"
    echo
...
Commands:
  ...
  uninstall   Remove VPNShroud completely
